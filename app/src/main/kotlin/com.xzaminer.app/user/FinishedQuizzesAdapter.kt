...
import com.xzaminer.app.utils.QB_COMPLETED
...
class FinishedQuizzesAdapter(
    activity: ListFinishedQuizzesActivity, var quizzes: ArrayList<StudyMaterial>, recyclerView: MyRecyclerView, fastScroller: FastScroller? = null,
    itemClick: (Any) -> Unit) :
        MyRecyclerViewAdapter(activity, recyclerView, fastScroller, itemClick) {

    private var currentQuizzesHash = quizzes.hashCode()
    ...
    fun updateQuizzes(newQuizzes: ArrayList<StudyMaterial>) {
        ...
        if (newQuizzesClone.hashCode() != currentQuizzesHash) {
            currentQuizzesHash = newQuizzesClone.hashCode()
    ...
    private fun setupView(view: View, quiz: StudyMaterial) {
        view.apply {
            ...
            vid_desc.text = "Result: " + quiz.fetchResult() + "%\nCompleted On: " + quiz.properties[QB_COMPLETED]?.first()
            vid_image.beGone()
